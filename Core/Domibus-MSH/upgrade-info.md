...
### DB upgrade script
                ...
                - Run the appropriate DB upgrade script:
                    o [Oracle only]
                        - single tenancy: oracle-5.0.5-to-5.1-upgrade.ddl, oracle-5.0.5-to-5.1-data-upgrade.ddl
                        - multitenancy:
                            - general schema: oracle-5.0.5-to-5.1-multi-tenancy-upgrade.ddl
                            - domain schemas: oracle-5.0.5-to-5.1-upgrade.ddl, oracle-5.0.5-to-5.1-data-upgrade.ddl
                  ...
                  o [MySQL only]
                      The scripts below - please adapt to your local configuration (i.e. users, database names) - can be run using either:
                          - the root user, specifying the target databases as part of the command. For example, for single tenancy:
                                  mysql -u root -p domibus < mysql-5.0.5-to-5.1-upgrade.ddl
                                  mysql -u root -p domibus < mysql-5.0.5-to-5.1-data-upgrade.ddl
                              or, for multitenancy:
                                  mysql -u root -p domibus_general < mysql-5.0.5-to-5.1-multi-tenancy-upgrade.ddl
                                  mysql -u root -p domibus_domain_1 < mysql-5.0.5-to-5.1-upgrade.ddl
                                  mysql -u root -p domibus_domain_1 < mysql-5.0.5-to-5.1-data-upgrade.ddl
                          - the non-root user (e.g. edelivery): for which the root user must first relax the conditions on function creation by granting the SYSTEM_VARIABLES_ADMIN right to the non-root user:
                            ...
                            and then specifying the target databases as part of the command. For example, for single tenancy:
                                   mysql -u edelivery -p domibus < mysql-5.0.5-to-5.1-upgrade.ddl
                                   mysql -u edelivery -p domibus < mysql-5.1-data-upgrade.ddl
                               or, for multitenancy:
                                   mysql -u edelivery -p domibus_general < mysql-5.0.5-to-5.1-multi-tenancy-upgrade.ddl
                                   mysql -u edelivery -p domibus_domain_1 < mysql-5.0.5-to-5.1-upgrade.ddl
...
#### Situation B: upgrading an existing 5.0.4 database, that contains user messages and was not partitioned
               - Run as edelivery_user:
    @oracle-5.0.2-to-5.0.5-upgrade.ddl
